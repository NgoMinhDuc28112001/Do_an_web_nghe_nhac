<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET')
    {
        if(!empty($_SESSION['user']))
        {
            ...
            and (songs.songName like '%{$searchTerm}%' or artists.artistName like '%{$searchTerm}%')
            ...
            inner join artists on songartist.artistID = artists.id
            LEFT JOIN likesong ON likesong.songID = songs.id 
            where songs.id NOT IN (SELECT songID FROM likesong WHERE userID = {$_SESSION['user']['id']})
            and (songs.songName like '%{$searchTerm}%' or artists.artistName like '%{$searchTerm}%') 
            ";
                ...
                echo "
                    ...
                    for($i = 0; $i < count($newResult); $i++)
                    {
                                ...
                                <li class='header__container-items-result-items'>
                                    <div class='header__container-items-result-items-left'>
                                        ...
                                        <div class='header__container-items-result-items-block'>
                                            ...
                                            <div id='blur-search-{$newResult[$i]['id']}' class='header__container-items-result-items-block-blur'></div>
                                            <div id='block-icon-large-{$newResult[$i]['id']}' class='header__container-items-result-items-block-icon'>
                                                <div id='block-icon-{$newResult[$i]['id']}' class='header__container-items-result-items-block-icon-i'>
                        ...
                        echo "
                                        ...
                                        <div class='header__container-items-result-items-right-button'>
                                            <i title='Thêm vào media player' class='icon ic-add-play-now icon--active'></i>
                                        </div>
                                        <div class='header__container-items-result-items-right-button'>
                                            <i title='Tải bài hát về' class='icon ic-download icon--active'></i>
                                        </div>
                                    </div>
            ...
            $sql = "SELECT
                ...
                inner join songartist on songs.id = songartist.songID
                inner join artists on songartist.artistID = artists.id
                where songs.songName like '%{$searchTerm}%' or artists.artistName like '%{$searchTerm}%'
            ";
